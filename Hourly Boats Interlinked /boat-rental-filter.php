...
//     $selectedDate = sanitize_text_field($_POST['date']); // Format: Y-m-d

//     $blocked_rows = get_field('blocked_dates', 'option') ?: [];
...
//         if (!empty($row['date'])) {
//             $blocked_dates[] = $row['date']; // Assuming 'date' is in Y-m-d format
//         }
//     }

//     // Start from selected date, and check for next available date
//     $nextAvailableDate = $selectedDate;
//     $maxCheckDays = 30; // Avoid infinite loops, stop after 30 days
//     $counter = 0;

//     while (in_array($nextAvailableDate, $blocked_dates)) {
//         $nextAvailableDate = date('Y-m-d', strtotime($nextAvailableDate . ' +1 day'));
//         $counter++;
...
//     // Now $nextAvailableDate contains either the selected date or next available one
...
function brf_plugin_fetch_time_slots() {
    ...
    $blocked_rows = get_field('blocked_dates', 'option') ?: [];
    // $blocked_dates = array_column($blocked_rows, 'date'); // Extract dates in Y-m-d format

    // Find next available date
    // $nextAvailableDate = $selectedDate;
    // $maxCheckDays = 30; // Avoid infinite loops
    // $counter = 0;
    ...
    foreach ($blocked_rows as $row) {
        $date = $row['date'];
        $type = $row['applies_to'];
        if (($rental_type === 'hourly' && ($type === 'hourly' || $type === 'both')) ||
            ($rental_type === 'drop_off' && ($type === 'drop_off' || $type === 'both'))) {
            $blocked_dates[] = $date;
        }
    }
    ...
    // Get interlinked boat ID
    $interlinked_boat_id = get_field('interlinked_boat', $boat_id);
    $interlinked_boat_id = is_object($interlinked_boat_id) ? $interlinked_boat_id->ID : $interlinked_boat_id;
    $is_interlinked = !empty($interlinked_boat_id);

    // Fetch all boats with the same interlinked_boat_id
    $related_boats = [];
    if ($is_interlinked) {
        $related_boats_query = new WP_Query([
            'post_type' => 'boat-rental',
            'posts_per_page' => -1,
            'meta_query' => [
                [
        ...
        while ($related_boats_query->have_posts()) {
            $related_boats_query->the_post();
            $related_boats[] = [
                'id' => get_the_ID(),
                'title' => get_the_title(),
                'booking_hours' => intval(get_field('booking_hours', get_the_ID())) ?: 2
            ];
        }
        wp_reset_postdata();
    } else {
        $related_boats[] = [
            ...
            'booking_hours' => intval(get_field('booking_hours', $boat_id)) ?: 2
        ];
    }

    // Set total boats capacity to 2 (shared pool)
    $total_boats_capacity = 2;

    // Get booking hours from the requested boat
    $requested_booking_hours = intval(get_field('booking_hours', $boat_id)) ?: 2;

    // Collect all bookings for boats with the same interlinked_boat_id
    $boat_ids = array_column($related_boats, 'id');
    $booking_query = new WP_Query([
        'post_type' => 'booking',
        'posts_per_page' => -1,
        'meta_query' => [
            ['key' => 'boat_id', 'value' => $boat_ids, 'compare' => 'IN'],
            ['key' => 'booking_date', 'value' => $selectedDate, 'compare' => '='],
            ['key' => 'booking_status', 'value' => 'confirmed', 'compare' => '='],
        ]
    ]);
    ...
    while ($booking_query->have_posts()) {
        $booking_query->the_post();
        $time_slot = get_post_meta(get_the_ID(), 'time_slot', true);
        $booking_hours = intval(get_post_meta(get_the_ID(), 'booking_hours', true));
        if (!$booking_hours) {
            $booking_hours = intval(get_field('booking_hours', get_post_meta(get_the_ID(), 'boat_id', true))) ?: 2;
            error_log("Warning: booking_hours not found for booking ID " . get_the_ID() . ", using fallback: " . $booking_hours);
        }
        $booked_boat_id = intval(get_post_meta(get_the_ID(), 'boat_id', true));
        $start_time = strtotime($time_slot);
        $end_time = strtotime("+$booking_hours hours", $start_time);
        $booked_time_ranges[] = [
            'start' => $time_slot,
            'start_time' => date('Y-m-d H:i:s', $start_time),
            'end_time' => date('Y-m-d H:i:s', $end_time),
            'type' => 'hourly',
            'duration' => $booking_hours,
            'boat_id' => $booked_boat_id
        ];
    }
    wp_reset_postdata();
    error_log("Booked Time Ranges: " . print_r($booked_time_ranges, true));

    // Process time slots for the requested boat
    if (have_rows('boat_time_slots', $boat_id)) {
        while (have_rows('boat_time_slots', $boat_id)) {
            the_row();
            $time_slot = get_sub_field('time_slot');
            $slot_time = strtotime($time_slot);
            $requested_booking_hours = intval($_POST['duration']) ?: intval(get_field('booking_hours', $boat_id)) ?: 2;
            $slot_end_time = strtotime("+$requested_booking_hours hours", $slot_time);
    
            $total_booked_count = 0;
            $booked_boat_ids = [];
            $min_booked_start = PHP_INT_MAX;
            $max_booked_end = $slot_time;
    
            for ($check_time = $slot_time; $check_time < $slot_end_time; $check_time += 1800) {
                $boats_in_slot = [];
                foreach ($booked_time_ranges as $range) {
                    $range_start = strtotime($range['start_time']);
                    $range_end = strtotime($range['end_time']);
                    if ($check_time >= $range_start && $check_time < $range_end) {
                        $boats_in_slot[$range['boat_id']] = true;
                    }
                }
                $total_booked_count = max($total_booked_count, count($boats_in_slot));
            }
    
            $boats_available_count = $total_boats_capacity - $total_booked_count;
            $is_available = $boats_available_count > 0;
    
            $time_slots[] = [
                'time' => $time_slot,
                'available' => $is_available,
                'boat_quantity' => $total_boats_capacity,
                'boats_available' => max(0, $boats_available_count),
                'booking_hours' => $requested_booking_hours . ' hours',
                'booked_count' => $total_booked_count
            ];
        }
    }
    ...
    wp_send_json_success([
        ...
        'boats_available' => $total_boats_capacity,
        'slots' => $time_slots
    ]);
}
add_action('wp_ajax_brf_plugin_fetch_time_slots', 'brf_plugin_fetch_time_slots');
...
function brf_validate_booking($boat_id, $date, $time_slot, $booking_type = 'hourly') {
    $boat_hours = intval(get_field('booking_hours', $boat_id)) ?: 2; // Reference booking_hours from boat-rental
    $interlinked_boat_id = get_field('interlinked_boat', $boat_id);
    $interlinked_boat_id = is_object($interlinked_boat_id) ? $interlinked_boat_id->ID : $interlinked_boat_id;

    // Get all boats with the same interlinked_boat_id
    $boat_ids = [$boat_id];
    if ($interlinked_boat_id) {
        $related_boats_query = new WP_Query([
            'post_type' => 'boat-rental',
    ...
    // Set total boats capacity to 2 (shared pool per booking_hours)
    $total_boats_available = 2;

    $start_time = strtotime($time_slot);
    $end_time = strtotime("+$boat_hours hours", $start_time);

    // Collect booked ranges with booking_hours from booking post type
    $booked_ranges = [];
    $booking_query = new WP_Query([
        'post_type' => 'booking',
        'posts_per_page' => -1,
    ...
    while ($booking_query->have_posts()) {
        $booking_query->the_post();
        $booked_boat_id = intval(get_post_meta(get_the_ID(), 'boat_id', true));
        $booked_boat_hours = intval(get_post_meta(get_the_ID(), 'booking_hours', true)) ?: $boat_hours; // Use booking_hours from booking
        $slot = get_post_meta(get_the_ID(), 'time_slot', true);
        $booked_start = strtotime($slot);
        $booked_end = strtotime("+$booked_boat_hours hours", $booked_start);
        $booked_ranges[] = ['start' => $booked_start, 'end' => $booked_end];
    }
    wp_reset_postdata();

    // Check availability for the requested window, preventing overlap
    $max_booked = 0;
    for ($check_time = $start_time; $check_time < $end_time; $check_time += 1800) {
        $booked_in_slot = 0;
        foreach ($booked_ranges as $range) {
        ...
        $max_booked = max($max_booked, $booked_in_slot);
        if ($max_booked >= $total_boats_available) {
            return false; // Prevent booking if max capacity is reached
        }
    }

    return $max_booked < $total_boats_available;
}